...
find_shows() {
    readarray -t monitoredSeries < <(curl -sSH "${HEADER}" "${API_URL}/series" | jq -c '.[] | select(.monitored == true) | {id,title}')
...
search_episodes() {
    ...
    for serie in "${monitoredSeries[@]}"; do
        ...
        readarray -t episodesTBA < <(curl -sSH "${HEADER}" "${API_URL}/episodeFile?seriesId=${seriesId}"| jq --arg seriesTitle "${seriesTitle}" -c '.[] | select(.relativePath | test("TBA|Episode [0-9]{1,}")) | . += { seriesTitle: $seriesTitle } | {id,seriesId,seriesTitle,relativePath}')
...
refresh_series() {
    ...
    for serie in "${seriesToRefresh[@]}"; do
        ...
        curl -sSH "${HEADER}" -X POST "${API_URL}/command" -d "{\"name\":\"RefreshSeries\",\"seriesId\":${seriesId}}" -o /dev/null
        printf " ✓\\n"
    done
}
...
wait_working() {
    ...
    for i in {1..30}; do sleep 1; printf "."; done
    echo ""
}
...
rename_episodes() {
    ...
    for serie in "${seriesToRefresh[@]}"; do
        ...
        printf "${YELLOW}${seriesTitle}"
        curl -sSH "${HEADER}" -X POST "${API_URL}/command" -d "{\"name\":\"RenameFiles\",\"seriesId\":${seriesId},\"files\":[${episodesList}]}" -o /dev/null
        printf " ✓\\n"
    done
}
